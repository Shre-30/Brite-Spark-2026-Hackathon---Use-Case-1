"""
Retrieval against the AFTER (pre-amendment) corpus. Same TF-IDF + Porter
stemming + sub-item indexing logic as retriever.py, just pointed at
data/chunks_after.json instead of data/chunks.json.
"""
...
CHUNKS_PATH = "data/chunks_after.json"
...
def build_index():
    ...
    print(f"[AFTER] Loaded {len(chunks)} clauses ({len(entries)} searchable entries) from {CHUNKS_PATH}")
    return chunks
