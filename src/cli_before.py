"""
Full pipeline CLI against the BEFORE (pre-amendment) corpus.
...
import retriever_before as retriever
...
def answer_question(question: str, k: int = 5, verbose: bool = False) -> str:
    ...
    if verbose:
        print("--- [BEFORE] retrieved clauses ---", file=sys.stderr)
        for r in retrieved:
...
def main():
    parser = argparse.ArgumentParser(description="[BEFORE] Grounded policy-manual assistant")
    ...
    print("[BEFORE] Grounded Answer — interactive mode. Ctrl+C or empty line to exit.\n")
    try:
        while True:
